...
if ($agent_id == $user_id) {
    Response::toast('合伙人不能是自己！', $this->createWebUrl('agent', ['op' => 'partner', 'id' => $agent_id]), 'error');
}
...
if ($user->isAgent() || $user->isPartner()) {
    Response::toast('该用户已经是代理商或合伙人！', $this->createWebUrl('agent', ['op' => 'partner', 'id' => $agent_id]), 'error');
}
