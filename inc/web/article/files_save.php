...
if ($id) {
    ...
    if (empty($archive)) {
        Response::toast('找不到这个附件！', $this->createWebUrl('article', ['op' => 'files']), 'error');
    }
}
...
if (empty($data['title']) || empty($data['url'])) {
    Response::toast('请填写标题和网址！', $this->createWebUrl('article', ['op' => 'files']), 'error');
}
...
if ($archive && $archive->save()) {
    Response::toast('保存成功！', $this->createWebUrl('article', ['op' => 'files']), 'sucess');
}

Response::toast('保存失败！', $this->createWebUrl('article', ['op' => 'files']), 'error');