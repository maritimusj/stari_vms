require(['jquery', 'util', vuejsUrl, echartsUrl, "clipboard"], function ($, util, Vue, echarts, ClipboardJS) {
    ...
    new Vue({
        ...
        watch: {
            'device.page': function (page) {
                this.getDevicesList();
            },
            'device.filter.online': function (v) {
                if (v) {
                    this.device.filter.offline = 0;
                }
            },
            'device.filter.offline': function (v) {
                if (v) {
                    this.device.filter.online = 0;
                }
            },
            expendStatus: function (v) {
            ...
            getDevicesList() {
                ...
                $.get(config.deviceApiUrl, Object.assign({}, self.device.lastParams, {
                    ...
                    if (res && res.status && res.data) {
                        ...
                        const ids = self.device.list.map(function (obj) {
                            return obj.id;
                        });
                        $.post(config.deviceApiUrl, {op: "online_stats", ids: ids}).then(function (res) {
                            if (res && res.status && res.data) {
                        ...
                        $.post(config.deviceApiUrl, {op: "device_data", ids: ids}).then(function (res) {
                            if (res && res.status && res.data) {
                                res.data.forEach(data => {
                                    const index = self.device.list.findIndex(device => device.id == data.id);
                                    if (index != -1) {
...
api.deviceRemove = function (id) {
    if (confirm('确定要删除这个设备吗？')) {
        api.showResult({op: 'remove', id: id}, null, false, function (res) {
            if (res.type === 'success') {
                $('tr[data-fetchid=' + id + ']').fadeOut("normal", function () {
                    $(this).remove();
                })
            }
        })
...
api.showMonthStats = function (deviceid, params, self) {
    const month = self.data('month');
