    ...
    $.getJSON(url || api.url, params).done(function (res) {
        if (res) {
            ...
            if (res.data && res.data.message) {
                util.message(res.data.message, '', res.status ? 'success' : 'error');
            }
...
require(['jquery', 'util', vuejsUrl, echartsUrl, "clipboard"], function ($, util, Vue, echarts, ClipboardJS) {
    ...
    const app = new Vue({
        ...
        methods: {
            openDoor(id, index) {
                $.get(config.deviceApiUrl, {op: 'openDoor', id, index}).then(res => {
            ...
            incrPageSize() {
                this.device.pagesize = this.device.pagesize * 2;
                this.getDevicesList();
            },
            tagClicked(tag) {
            ...
            downloadQRCode() {
                util.loading();
                $.post(config.deviceApiUrl, {op: 'download_qrcodes', ids: this.selectedInputCtrl}).then(res => {
                    util.loaded();
            ...
            deleteAllDevice() {
                const self = this;
                if (self.selectedInputCtrl.length == 0) {
                    util.message('请选择要删除的设备！', '', 'error');
                    return;
                }
                
                if (!confirm('确定要删除所有选中的设备吗？注意：删除操作无法撤回！')) {
                    return;
                }
                
                self.selectedInputCtrl.forEach(id => {
                    $.get(config.deviceApiUrl, {op: 'remove', id}).then(res => {
                        if (res && res.status) {
                            self.device.list = self.device.list.filter(e => e.id != id);
                            self.selectedInputCtrl = self.selectedInputCtrl.filter(i => i != id);
                            if (self.selectedInputCtrl.length == 0) {
                                self.getDevicesList({}, true);
                            }
                        }
                    })
                })
            ...
            tagsPrePage() {
                if (this.tags.page > 1 && !this.tags.loading) {
                    this.tags.page -= 1;
                    this.getTagsList();
                }
            },
            ...
            getListBy(w) {
                ...
                this.sort.by = w;
                this.getDevicesList(this.sort);
            },
            getDevicesList(params = {}, loading = false) {
                const self = this;
                self.device.loading = true;
                self.device.lastParams = self.getParams();
                loading && util.loading();
                $.get(config.deviceApiUrl, Object.assign({}, self.device.lastParams, Object.assign({
                    op: "list",
                    page: self.device.page,
                    pagesize: self.device.pagesize,
                }, params)), function (res) {
                    if (res && res.status && res.data) {
                        localStorage.setItem('device_list.page', self.device.page);
            ...
            fetchDeviceData(id) {
                id = parseInt(id);
                const device = this.device.list.find(device => device.id === id);
                if (device) {
                    device.device_data = undefined;
    ...
    api.uploadInfo = function () {
        if (confirm('确定要上传所有设备信息到第三方平台吗？')) {
            api.showResult({ op: 'upload_info' });
        }
    }
    api.exportSIM = function () {
        if (confirm('确定要导出全部SIM卡信息吗？')) {
            api.showResult({ op: 'export_sim' });
        }
    }    
    ...
    api.deviceTestLaneN = function (id, num, self) {
        ...
        if (self) {
            ...
            table.find('td.test_btn').each(function () {
                ...
                e.data('btn', e.html());
                if (e.is(self)) {
                    e.html('<div class="loading"><i class="fa fa-spinner fa-pulse"></i></div>');
                } else {
                    e.html('');
                }
            })
    ...
    api.deviceReset = function (id, lane, self) {
        const name = self.data('name') ? self.data('name') : '货道';
        let title;
        if (lane === undefined) {
            title = '确定要重置商品数量吗？';
        } else if (lane === 'all') {
            title = '确定要重置这台设备所有' + name + '的商品数量吗？';
        } else {
            title = '确定要重置 ' + name + (lane + 1) + ' 的商品数量吗？';
        }
    ...
    api.getScheduleTaskList = function (id) {
        api.showResult({ op: 'schedule', id: id })
    }

    api.createScheduleTask = function (id) {
        api.showResult({ op: 'schedule', id: id, fn: 'create' })
    }
    
    api.saveScheduleTask = function () {
        const id = $('#editScheduleForm input[name=deviceId]').val();

        let data = { op: 'schedule', id: id, fn: 'save'}

        const spec = $('#editScheduleForm input[name=spec]').val();
        if (spec != undefined) {
            if (spec == '') {
                util.message('请输入正确的CRON表达式！', '', 'error');
                return;
            }
            data.spec = spec;
        } else {
            const hour = $('#editScheduleForm input[name=h]').val();
            const minute = $('#editScheduleForm input[name=m]').val();
            const second = $('#editScheduleForm input[name=s]').val();
        ...
        api.showResult(data, null, function() {
            app.fetchDeviceData(id);
            return false;
        })
    }
    ...
    api.viewCardStatus = function (deviceid, _, self) {
        const iccid = self.closest('span').data('uid');
        api.showResult({ op: 'card_status', iccid });
    }

    api.start = function (deviceid, params, _) {
        api.showResult({ op: 'start', id: deviceid, chargerID: params });
    }

    api.stop = function (deviceid, params, _) {
        api.showResult({ op: 'stop', id: deviceid, chargerID: params });
    }
    
    api.downloadLaneQRCodes = function(deviceid) {
        util.loading();
        $.post(config.deviceApiUrl, {op: 'download_qrcodes', fn: 'lanes', ids: [deviceid]}).then(res => {
            util.loaded();
            if (res && res.status && res.data.url) {
                window.location.href = res.data.url;
            }
...
$(function () {
    ...
    $('body').on('click', '[data-op]', function (e) {
        ...
        if (op && api[op]) {
            const id = $(this).closest('[data-fetchid]').data('fetchid');
