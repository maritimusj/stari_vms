...
require(['jquery', 'util', vuejsUrl, echartsUrl, "clipboard"], function ($, util, Vue, echarts, ClipboardJS) {
    ...
    api.deviceSchedule = function (id) {
        api.showResult({ op: 'schedule', id: id })
    }

    api.saveSchedule = function () {
        $('#modal-message').modal('hide');

        const deviceId = $('#editScheduleForm input[name=deviceId]').val();
        const delaySeconds = $('#editScheduleForm input[name=delaySeconds]').val();
        
        api.showResult({ op: 'scheduleSave', id: deviceId,  delay: delaySeconds }, null, function() {
            app.fetchDeviceData(deviceId);
        })
    }
