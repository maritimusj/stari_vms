...
if (!We7::pdo_field_exists($tb_name.'_order', 'user_id')) {
    $sql = <<<SQL
ALTER TABLE `ims_zovye_vms_order` ADD `user_id` VARCHAR(32) NULL AFTER `goods_id`, ADD INDEX u1 (`uniacid`, `user_id`);
SQL;
    Migrate::execSQL($sql);
}
...
if (!We7::pdo_field_exists($tb_name.'_order', 'account_id')) {
    $sql = <<<SQL
ALTER TABLE `ims_zovye_vms_order` ADD `account_id` VARCHAR(32) NULL AFTER `goods_id`, ADD INDEX a1 (`uniacid`, `account_id`);
SQL;
    Migrate::execSQL($sql);
}
