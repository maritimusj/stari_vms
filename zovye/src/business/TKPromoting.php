...
class TKPromoting
{
    const REDIRECT_URL = 'https://cloud.tk.cn/tkproperty/nprd/S2023062001/?fromId=77973&channelCode=999999990004&cusType=3&device_id={device_imei}&extra={tk_user_uid}&utm_source=FPDJK16876224b9fa';
    ...
    public static function deviceReg(deviceModelObj $device): bool
    {
        ...
        $res = (new self($config['id'], $config['secret']))->reg($device);
    ...
    public static function confirmOrder(deviceModelObj $device, string $tk_order_no): bool
    {
        ...
        $res = (new self($config['id'], $config['secret']))->confirm($device, $tk_order_no);
